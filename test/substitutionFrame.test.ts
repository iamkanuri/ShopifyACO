...
test("every named rival carries a real recommendation count; score is demoted to proof", () => {
  ...
  // v2 CP4 reframe: the merchant-facing name is "AI buyer readiness", not "AI Visibility Score".
  // Asserted here so the score label can't silently drift back to the retired category name.
  assert.match(f.scoreProof, /AI buyer readiness of 8\/100/);
  assert.doesNotMatch(f.scoreProof, /Visibility Score/);
