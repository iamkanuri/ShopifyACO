...
function buildMarkdown(
  ...
  //
  // MERGE NOTE (v2.1): the frame structure comes from main; the "AI buyer readiness" label comes from
  // v2 CP4 (retire "AI Visibility Score" as the merchant-facing category). Both sides demote the score —
  // main structurally, v2 by name — so both are kept. The label is renamed in ONE place per surface,
  // including `scoreProof` in analysis/substitutionFrame.ts, so this page never calls the same number
