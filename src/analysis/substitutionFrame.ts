...
export function selectSubstitutionFrame(input: FrameInput): SubstitutionFrame {
  ...
  // "AI buyer readiness", not "AI Visibility Score" (v2 CP4 reframe). The number and its
  // documented formula are UNCHANGED — only the merchant-facing name of it. This sentence sits
  // directly under the score heading in every renderer, so the two must use the same words.
  const scoreProof =
    input.score != null
      ? `Your AI buyer readiness of ${input.score}/100 is the summary of this: ${own} of ${N} answers recommend ${brand}.`
