...
interface CapabilityAxis {
  key: string; reader: string; fate: "descoped" | "pinned";
  capability_confirmed: number; capability_total: number; capability_pct: number;
  occurrence: number; occurrence_total: number; occurrence_pct: number;
}
interface CapabilityBlock {
  measured_at: string; engine_version: string;
  disclosure_policy: string; reader_guidance: string;
  axes: CapabilityAxis[];
  cost_bar: { strict: number; raw: number; derivation: string };
  fate: { sentence: string; ratio: number };
  completion: string;
}

let capabilityCache: CapabilityBlock | null = null;
function loadCapabilityBlock(): CapabilityBlock {
  if (capabilityCache) return capabilityCache;
  const raw = fs.readFileSync(path.join(repoRoot, "standards", "capability-frequency.json"), "utf8");
  const b = JSON.parse(raw) as CapabilityBlock;
  // An INCOMPLETE artifact must not render. A block whose own generator could not make its
  // arithmetic reproduce is exactly the thing this page must never publish.
  if (b.completion !== "VERIFIED_CLEAN") {
    throw new Error(`capability-frequency.json is ${b.completion} — refusing to render it`);
  }
  if (!Array.isArray(b.axes) || b.axes.length < 3) {
    throw new Error("capability-frequency.json carries fewer than three axes — it ships whole or not at all");
  }
  capabilityCache = b;
  return b;
}
 ...
 *
 * ⚠️ THE FLAG IS NOW AN OPT-OUT, and the inversion is the publication. It was
 * `STANDARDS_SHOW_CAPABILITY_BLOCK=1` to show; it is now `=0` to hide. A block that
 * publishes only when a production env var is set would have been "published" in the repo
 * and dark on the site — the exact `PUBLISHED`-stopped-at-v1.2 defect this project recorded
 * at v3.5, where every gate was green because each checked the artifact and nothing asserted
 * the page was REACHABLE.
 *
 * ⚠️ EVERY FIGURE IS READ FROM `standards/capability-frequency.json`, generated by
 * `experiments/v4-0/build_capability_block.ts` from the frozen adjudicated table, the
 * occurrence artifact and the adversarial pass. Nothing here is typed. The generator refuses
 * to write if its arithmetic does not reproduce, and this function refuses to render an
 * artifact that is not `VERIFIED_CLEAN`.
 *
 * ⚠️ IT SHIPS WHOLE OR NOT AT ALL. Publishing the two settled rows while withholding the
 * open one was considered at v3.9 and REJECTED as selective disclosure: a table showing two
 * axes at "attacks well, occurs never" and silently omitting the third reads as a clean bill
 * of health, which is the pattern this site exists to oppose. The length check above enforces
 * it in code rather than in intention.
 *
 * The disclosure POLICY renders beside the outcome on purpose. The block was complete on
 * 2026-07-28 and held until the open axis had a fix or a recorded limitation; saying so is
 * what makes the delay legible as a rule rather than as convenience.
 */
export function renderCapabilityBlock(): string {
  if (process.env.STANDARDS_SHOW_CAPABILITY_BLOCK === "0") return "";
  const b = loadCapabilityBlock();
  const FATE_LABEL: Record<string, string> = {
    descoped: "not guarded — it does not occur",
    pinned: "known limitation",
  };
  const rows = b.axes.map((a) => `<tr>
      <td>${esc(a.reader)}</td>
      <td>${a.capability_pct}% <span class="std-eg">(${a.capability_confirmed} of ${a.capability_total})</span></td>
      <td>${a.occurrence_pct}% <span class="std-eg">(${a.occurrence} of ${a.occurrence_total})</span></td>
      <td>${esc(FATE_LABEL[a.fate] ?? a.fate)}</td>
    </tr>`).join("");

  return `<section class="std-capability" id="capability-frequency">
  <h2>What a passing claim row cannot rule out</h2>
  ${p(b.reader_guidance)}
  ${p("We attack our own claim matcher with sentences written to break it. That measures CAPABILITY — what an adversary could do — and is deliberately not a measurement of what merchants write. Beside each capability figure is how often the same shape occurs in the sentences the engine actually rendered as evidence on real stores.")}
  <table class="std-bounds"><caption>Attack shape, by capability and by real-copy frequency</caption>
    <thead><tr>
      <th scope="col">What the sentence does</th>
      <th scope="col">Succeeds on sentences written to defeat it</th>
      <th scope="col">Occurs in real evidence sentences</th>
      <th scope="col">Status</th>
    </tr></thead>
    <tbody>${rows}</tbody></table>
  <div class="std-limit">
    <h3>The known limitation, and what it cost to try to close it</h3>
    ${p(b.fate.sentence)}
    ${p(`The bar it had to clear was derived, not chosen: ${esc(b.cost_bar.derivation)}.`)}
  </div>
  ${p(b.disclosure_policy)}
  <p class="std-note">Measured ${esc(b.measured_at)} against engine ${esc(b.engine_version)}.</p>
</section>`;
}
