...
import { ASSUMED_INPUT_TOKENS, MAX_OUTPUT_TOKENS, estimateCostUsd } from "./engines/models.js";
...
/** Worst-case cost: every call uses max output tokens, across every adapter. */
export function estimateMaxCost(promptCount: number, adapters: EngineAdapter[]): number {
  let total = 0;
  for (const a of adapters) {
    total += promptCount * estimateCostUsd(a.model, ASSUMED_INPUT_TOKENS, MAX_OUTPUT_TOKENS);
  }
  return total;
}
