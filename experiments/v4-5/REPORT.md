...
**Decision 2′, ATTEMPTED instead — and REVERTED before the session ended. See "The two
adversarial passes" below; this paragraph records what was built and why, not what shipped.** The defect is not which source; it is
...
⚠️ **THAT TABLE IS THE PRE-REVERT READING, AND IT IS THE POINT OF THIS REPORT.** It is a
clean result — 2 stores corrected, nothing lost, nothing else moved — produced by an
instrument that was working correctly, over a change that was about to ship seven regression
classes. **After the revert the same A/B reads: 11 rows dropped, 0 cap moves, 0 rows
appeared, 0 other-kind changes, 2806 unchanged.** The corrected prices are gone with it.

**Frozen fetch corpus**, attribution isolated in a worktree at unmodified `main`:
pre-revert **10 cases closed, 0 newly opened**; **at HEAD, 8 closed, 0 newly opened**
(`mm-07 znn-01 znn-02 znn-03 znn-04 znn-09 znn-13 znn-15`). `mm-01`/`mm-02`/`mm-15` closed
only under the reverted half; `mm-07` and `znn-09` close because `evaluate` now re-tests the
zero and `zeroAwareMin` checks its domain.
...
### The funnel event — DONE

The standard route emitted `standard_test_requested` and nothing else, so the standard
layer's completion rate was zero *by construction*. Now emits
`standard_test_completed`/`standard_test_failed`, discriminated on `result.ok` rather than
the HTTP status (a non-applicable product is a legitimate 200). Verified in the database:
`requirements 10, evidenced 4, not_proven 5, duration_ms 14639`, matching the run.

---

## THE TWO ADVERSARIAL PASSES

Both agents worked from a real `git worktree` at the parent commit, ran no package
manager, and put a two-sided canary in every probe. **Adv2 independently reproduced every
number in this report** — 335 stores, 2804 unchanged rows, 13/2 disappeared/appeared, 11
dropped, 10 false `$0.00`, 2 corrected, 0 other-kind changes, 10/0 corpus closed/opened.

What they found that changed what shipped:

| finding | outcome |
|---|---|
| `num()` coerces junk to 0, so a minimum lets a sibling offer beat a real price — 11 of 12 hostile values | **the offers-minimum was REVERTED** |
| the same change manufactures "zero beside a real price" on the offer path (226/397 pages) | reverted with it |
| `merchantFacts.ts` joins the variant matching the DISPLAYED price; a minimum is not that | reverted with it |
| `zeroAwareMin([0.004])` renders `$0.00` — the exact sentence the fix exists to stop | **fixed** (the guard tests cents, what renders) |
| `[-5]` answers "every price is zero"; `[Infinity]` renders `$Infinity` | **fixed** (domain checked here, not assumed from the caller) |
| the corpus case pinning P-19 is vacuous — it constructs `minPriceUsd: 0` directly | **fixed** (`evaluate` re-tests the zero; gaps 60 → 59) |
| **`npm run typecheck` is RED** — the standards project, which root `tsc` does not cover | **fixed** |
| `defect_classes` sums to 18 against a confirmed count of 5, LIVE on the published page | **fixed**, with a test |
| dropping a price candidate CAN admit a lower-ranked row — 6 snapshots at the 10-cap | **filed** (0 occurred here; the generalisation was false) |
| `p19_ab.mjs`'s `OTHER_KIND_CHANGES` reads edits only, so an *appeared* row could not reach it | acknowledged — the bare `rows_appeared` count is what carried it |
| removing a worktree traversed the `node_modules` junction and deleted `.bin/` twice | restored; the hazard is now recorded |

---

## PHASE B — PARTIALLY DONE, AND THE REMAINDER IS NAMED

| item | state |
|---|---|
| `outreach_final.md` regenerated from post-A2 state | **DONE** — `VERIFIED_CLEAN`, 0 unfilled placeholders, carrying 3.05% |
| fresh one-pager + result artifacts on a real prospect | **DONE** (the Phase C walk) |
| every pre-v4.3 one-pager / OG image regenerated | **NOT DONE** |
| thirdocular.com parent retheme to the light token system | **NOT DONE** (the repo is present at `../ThirdOcular`) |
| exhibits stamped with engine version + run date | **NOT DONE** |

The generator's own honesty flags are worth reading: it detected that **CLAUDE.md's 5.17%
was superseded** by the artifact's 3.05% and used the artifact (now fixed in CLAUDE.md);
that *"of 100"* is false for one of three findings; and that the pack's Track A argument
**inverts** on this store — every unmet requirement is one most peers also fail (11/100,
8/100, 7/99). It kept the numbers and rewrote the line, which is the rule.

---

## PHASE C — THE RUNWAY WALK, IN PRODUCTION, CLICK BY CLICK

Against `birdrockcoffee.com`, a real prospect not used as a demo, at commit `b728f0c`:

| step | result |
|---|---|
| 1. `POST /api/product-test/standard` | **200 in 15.2s** — 4 proven · 5 not proven · 1 requires store access, of 10 |
| 2. `GET /result/t_8ebd1e0da2eaa14de9d1` | **200, 13,692 bytes** — stamped Engine **v2.6.0**, Standard **ALS-COFFEE v1.3**, hash `ba2050578ed02748…`, contract `c1s-f3967d50` |
| 3. `GET /result/…/one-pager` | **200, 7,446 bytes** |
| 4. outreach message, every figure derived | **`VERIFIED_CLEAN`**, 0 unfilled placeholders, 3 honesty flags raised and kept |

Headers on the sendable artifact: `x-robots-tag: noindex, nofollow` · `cache-control:
private, no-store` · `referrer-policy: same-origin`.

Production verification of this release: all **five** disclosed results render their notice
with the correct mechanism and the re-run offer; `/standards/coffee/1.3` renders 473 rows
and 3.05%; **no ratio is drawn on any of the four published versions**; no `[object Object]`,
no `undefined`; both probes green.

---

## THE DECLARATION

**There is no session left between this product and the first send.**

The loop above is not a plan — it ran, in production, in four steps, and every artifact it
produced is on disk. An agency can today: paste a prospect's product URL, get a verdict
against a published, content-hashed standard, send a permanent URL that never re-runs and
still resolves, attach a one-pager that carries its own correction notices, and write an
outreach message in which **no number is hand-typed**.

Three things are true and none of them blocks a send:

1. **Phase B's remaining exhibits are stale, not wrong.** Old one-pagers and OG images
   predate v4.3's demo change. They are superseded artifacts, not false ones, and the walk
   above mints fresh ones on demand — which is the path an actual send uses anyway.
2. **The parent site (thirdocular.com) is still on the old palette.** It is the marketing
   parent, not the artifact an agency sends. The two-site copy gate is green.
3. **Stripe is in TEST mode pending KYC.** That blocks getting *paid*, not getting *read*.

**What I would not do without the user:** send anything. Every artifact here is addressed
to a named third-party merchant, and choosing who receives outreach is not a call the
repository can make.

---

## POST-SIGNAL WORK — to be chosen by agency answers, not by internal audit

Parking these with that label is the point: the next session should be chosen by something
outside this repository.

- **Suite 1.1 authoring for G-15** (referent). Its precondition is that the acceptance
  suite tests the wrong sentences; it must be its own attended session and must not be the
  session that writes the guard.
- **P-23** — the `unscented` → `fragrance_free` equivalence, unmeasured.
- **P-18** — what a price row should PROMISE for a non-USD store, and the authenticated
  path's missing currency handling. Needs a live Shopify connection.
- **P-19's second half, if it returns** — harden `num()` to the contract `priceToUsd`
  already enforces FIRST; decide whether a selling-plan offer is the same commitment as the
  one-time purchase the `no_subscription` row asserts (it is not); keep `priceSpecification`
  narrow. The tombstone above `parseOffer` carries the order.
- **The requirement-backfill residual** — 6 snapshots at the 10-requirement cap would gain
  an unadjudicated `In stock and purchasable` row if their price were refused.
- **The embedded Shopify app** still leads with Tests and never names a published standard.
- **P-29 / the semantic tier** — if it ever returns, the temperature/determinism hardening
  is a precondition, and variance must be RE-VERIFIED at ≈0 rather than assumed from the
